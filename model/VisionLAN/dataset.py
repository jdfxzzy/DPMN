...
class lmdbDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_str = str_filt(word, self.voc_type)           # 替换不在规定类型中的字符为空格
...
class lmdbDataset_real(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            img_hr = buf2PIL(txn, img_hr_key, 'RGB')                # HR图像
            img_lr = buf2PIL(txn, img_lr_key, 'RGB')                # LR图像
...
class resizeNormalize(object):
    ...
    def __call__(self, img):
        ...
        if self.mask:
            mask = img.convert('L')                                 # 像素值占8位的黑白图像 
            thres = np.array(mask).mean()                           # 阈值是根据像素均值来的，论文有说明
            mask = mask.point(lambda x: 0 if x > thres else 255)
            mask = self.toTensor(mask)
            img_tensor = torch.cat((img_tensor, mask), 0)           # 原始图像和mask拼接
        return img_tensor
...
class alignCollate_syn(object):
    ...
    def __call__(self, batch):
        ...
        # 通过BICUBIC将图像先调整尺寸为原先的一半，相当于一个自己制造LR图像的过程
...
class alignCollate_real(alignCollate_syn):
    ...
    def __call__(self, batch):
        ...
        transform = resizeNormalize((imgW, imgH), self.mask)
        transform2 = resizeNormalize((imgW // self.down_sample_scale, imgH // self.down_sample_scale), self.mask)
        images_hr = [transform(image) for image in images_hr]           # HR图像尺寸调整为128*32
        images_hr = torch.cat([t.unsqueeze(0) for t in images_hr], 0)   # 按batch维度拼接为一个张量

        images_lr = [transform2(image) for image in images_lr]          # LR图像尺寸调整为64*16
        images_lr = torch.cat([t.unsqueeze(0) for t in images_lr], 0)
...
class ConcatDataset(Dataset):
    ...
    def __init__(self, datasets):
        ...
        self.cumulative_sizes = self.cumsum(self.datasets)          # 拼接数据集的总长度

    def __len__(self):
        return self.cumulative_sizes[-1]

    def __getitem__(self, idx):
        dataset_idx = bisect.bisect_right(self.cumulative_sizes, idx)   # 定位到在哪个数据集中
        # bisect类为Python中实现的二分搜索函数
        # bisect.bisect_right(a, x, [lo=0, hi=len(a)])的作用是：
        # 在序列 a 中二分查找适合元素 x 插入的位置，保证 a 仍为有序序列。
        # 若序列 a 中存在与 x 相同的元素，则返回与 x 相同的最后一个(最右侧)元素的位置索引，使得 x 若插入后能位于其右侧；
        # 若序列 a 中不存在与 x 相同的元素，则返回与 x 左侧距离最近的元素位置索引，使得 x 若插入后能位于其右侧。
        # 而 lo 和 hi 为可选参数，分别定义查找范围/返回索引的上限和下限，缺省时默认对整个序列查找。

        if dataset_idx == 0:
            sample_idx = idx
        else:
            sample_idx = idx - self.cumulative_sizes[dataset_idx - 1]   # 定位到是这个数据集的第几个样本
