...
class STNHead(nn.Module):
  def __init__(self, in_planes, num_ctrlpoints, activation='none', input_size=[32,64]):
    ...
    self.stn_convnet = nn.Sequential(
                          conv3x3_block(in_planes, 32),         # (N, 32, H, W)
                          nn.MaxPool2d(kernel_size=2, stride=2),
                          conv3x3_block(32, 64),                # (N, 64, H/2, W/2)
                          nn.MaxPool2d(kernel_size=2, stride=2),
                          conv3x3_block(64, 128),               # (N, 128, H/4, W/4)
                          nn.MaxPool2d(kernel_size=2, stride=2),
                          conv3x3_block(128, 256),              # (N, 256, H/8, W/8)
                          nn.MaxPool2d(kernel_size=2, stride=2),
                          conv3x3_block(256, 256),              # (N, 256, H/16, W/16)
                          nn.MaxPool2d(kernel_size=2, stride=2),
                          conv3x3_block(256, 256))              # (N, 256, H/32, W/32)
  ...
  def forward(self, x):
    ...
    输入：(N, C, H, W)
    """
    x = self.stn_convnet(x)                 # (N, 256, H/32, W/32)
    batch_size, _, h, w = x.size()
    x = x.view(batch_size, -1)              # (N, HW/4)
    # embed()
